...
const exportTaskReport = async (req, res) => {
    try {
        ...
        tasks.forEach((task) => {
            ...
            worksheet.addRow({_id: task._id, title: task.title, description: task.description, priority: task.priority, status: task.status, dueDate: task.dueDate.toISOString().split("T")[0], assignedTo: assignedTo || "Unassigned", });
        });
...
const exportUserReport = async (req, res) => {
    try {
        ...
        const userTaskMap = {};
        users.forEach((user) => {
            userTaskMap[user._id] = { name: user.name, email: user.email, taskCount: 0, pendingTasks: 0,
